/**
 * Fetches C1/C2 vocabulary from verbs.fly-work.com and imports into the local DB.
 *
 * Usage:
 *   node scripts/import-c1-c2-words.js
 *
 * Env (.env / .env.local):
 *   DATABASE_URL
...
async function main() {
  ...
  for (const { code, order } of LEVELS) {
    ...
    console.log(`Level ${code}: id=${levelRecord.id}, order=${order}`);

    console.log(`Fetching ${code} from ${API_BASE_URL}...`);
    const words = await fetchAllWordsForLevel(code);
    console.log(`  ${words.length} word(s) from API`);
    if (words.length === 0) {
      console.warn(
        `  ⚠ API returned no words for ${code}. verbs.fly-work.com may not have ${code} vocabulary yet (A1–B2 often have data).`,
      );
    }
    ...
    console.log(
      `  Import ${code}: inserted=${stats.inserted}, skipped=${stats.skipped}, fetched=${stats.fetched}`,
    );
  }
