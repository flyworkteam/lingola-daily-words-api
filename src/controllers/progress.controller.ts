...
import {
  ...
  type CalculateAndApplyDailyRewardsResult,
  type DailyRewardIncrementType,
  type IncrementDailyRewardProgressInput,
...
function sendProgressSuccess(
  res: Response,
  progress: unknown,
  rewardResult?: CalculateAndApplyDailyRewardsResult | null,
) {
  if (rewardResult) {
...
function isFirstTimeLearned(
  existing: { status: string; learnedAt: Date | null } | null | undefined,
  shouldMarkLearned: boolean,
): boolean {
...
function resolveAnswerSource(body: z.infer<typeof answerBodySchema>) {
  if (body.source) {
    return body.source;
  }
...
async function applyAnswerDailyRewards(
  userId: string,
  source: 'test' | 'speaking' | 'review' | undefined,
  isCorrect: boolean,
  isNewlyLearned: boolean,
): Promise<CalculateAndApplyDailyRewardsResult | null> {
  const increments: IncrementDailyRewardProgressInput[] = [];
...
async function assertActiveVocabularyItem(vocabularyItemId: string) {
  return prisma.vocabularyItem.findFirst({
    where: { id: vocabularyItemId, isActive: true },
    select: { id: true },
  });
}

async function recordDailyActivitySafely(
  deltas: Omit<DailyActivityDeltas, 'userId'>,
  userId: string,
  context: string,
) {
  try {
...
function mapLegacyRewardType(
  type: z.infer<typeof dailyRewardRecordSchema>['type'],
): DailyRewardIncrementType | 'review_batch' {
  switch (type) {
    case 'learned_word':
...
export async function markVocabularySeen(req: Request, res: Response) {
  try {
    const user = getAuthUser(req, res);
    if (!user) return;

    const { id: vocabularyItemId } = req.params;
    const vocabularyItem = await assertActiveVocabularyItem(vocabularyItemId);
    if (!vocabularyItem) {
...
export async function toggleVocabularySave(req: Request, res: Response) {
  try {
    const user = getAuthUser(req, res);
    if (!user) return;
...
export async function submitVocabularyAnswer(req: Request, res: Response) {
  try {
    const user = getAuthUser(req, res);
    if (!user) return;
    ...
    const updateData: Prisma.UserVocabularyProgressUpdateInput = {
      seenCount: { increment: 1 },
      lastSeenAt: now,
    ...
    if (answerTimeMs !== undefined) {
      updateData.totalAnswerTimeMs = { increment: answerTimeMs };
    }

    if (shouldMarkLearned) {
      updateData.status = 'learned';
      updateData.learnedAt = existing?.learnedAt ?? now;
    }
    ...
    const progress = await prisma.userVocabularyProgress.upsert({
      ...
      create: {
        ...
        learnedAt: shouldMarkLearned ? now : null,
      },
      update: updateData,
      include: progressInclude,
    });
    ...
    await recordDailyActivitySafely(
      isCorrect
        ? { seenDelta: 1, correctDelta: 1 }
        : { seenDelta: 1, wrongDelta: 1 },
      user.id,
...
export async function getMyAdaptiveLevel(req: Request, res: Response) {
  try {
    const user = getAuthUser(req, res);
    if (!user) return;
...
export async function applyMyAdaptiveLevel(req: Request, res: Response) {
  try {
    const user = getAuthUser(req, res);
    if (!user) return;
...
export async function getMyDailyProgress(req: Request, res: Response) {
  try {
    const user = getAuthUser(req, res);
    if (!user) return;
...
export async function postDailyActivity(req: Request, res: Response) {
  try {
    const user = getAuthUser(req, res);
    if (!user) return;
...
export async function getMyDailyRewardProgress(req: Request, res: Response) {
  try {
    const user = getAuthUser(req, res);
    if (!user) return;
...
export async function postDailyRewardRecord(req: Request, res: Response) {
  try {
    const user = getAuthUser(req, res);
    if (!user) return;
...
export async function getMyVocabularyProgress(req: Request, res: Response) {
  try {
    const user = getAuthUser(req, res);
    if (!user) return;
